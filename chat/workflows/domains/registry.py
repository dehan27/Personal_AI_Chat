...
class WorkflowEntry:
    ...
    `input_schema` 는 `workflow_input_extractor` 가 자연어 질문에서 어떤
    필드를 어떤 타입으로 뽑을지 결정하는 근거 (Phase 6-2). 기본값 빈 dict 라
    스키마가 없는 workflow 는 extractor 가 `{}` 를 반환해 기존처럼
    `run_workflow` 가 알아서 MISSING_INPUT 을 낸다.
    """
    key: str
    title: str
    description: str
    ...
    input_schema: Mapping[str, FieldSpec] = field(default_factory=dict)
