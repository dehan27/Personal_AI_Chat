...
from chat.services.agent.result import AgentTermination, SourceRef
...
class Observation:
    ...
    Phase 8-1:
    - `arguments` — LLM 이 시도한 호출 인자. trace 가 observations 만으로 step 별
      재구성 가능하게 함 (state.tool_calls 와 의 1:1 보장 X 이슈 해소).
    - `evidence` — 도구 결과의 source 메타 (예: retrieve_documents 의 top-1 hit
      의 SourceRef). `tools.call` 이 result dict 의 `'evidence'` 키에서 자동 부착.
    """
    tool: str
    summary: str
...
class AgentState:
    ...
    def add_observation(
        ...
        *,
        ...
        Phase 8-1: `arguments` / `evidence` 로 trace 재구성 + sources 흐름 지원.
        """
        obs = Observation(
