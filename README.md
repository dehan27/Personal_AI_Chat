...
- `agent/` — generic agent runtime (Phase 7-1) + graph 결선 (Phase 7-2) + snippet windowing (Phase 7-3). ReAct loop 가 등록된 도구(`retrieve_documents` / `find_canonical_qa` / `run_workflow`) 를 반복 호출하며 multi-hop 탐색형 질문을 처리. 진입점은 `chat.services.agent.react.run_agent(question, history) -> WorkflowResult`, 사용자-facing reply 변환은 `chat.services.agent.reply.build_reply_from_agent_result`. `Tool.input_schema` 는 `Optional[Mapping[str, FieldSpec]]` 로 schema 모드 / raw 모드 두 갈래 (`run_workflow` 는 raw — workflow_key 마다 input 형태가 달라서). 종료 정책은 `max_iterations=6` (7-2 smoke 가 4 부족을 잡아내 상향), 연속 실패 한도(3), 동일 호출 반복 한도(3). 결과는 Phase 5 `WorkflowResult` 그대로 재사용 — 새 dataclass 없음. `AgentTermination` 내부 사유는 `to_workflow_result(...)` 가 `OK / NOT_FOUND / UPSTREAM_ERROR` 중 하나로 매핑한다 (`UNSUPPORTED` 는 라우팅 단의 책임이라 agent 가 만들지 않음; reply 단도 ValueError 로 봉쇄). Phase 7-2 부터 `chat/graph/nodes/agent.py` 가 `ROUTE_AGENT` 진입점 — history-aware rewrite → `run_agent` → reply → `QueryResult`. graph 결선 자동 회귀 테스트(`test_graph_agent_wiring.py`) 가 ROUTE_* 세 분기를 `_compiled_graph.cache_clear()` + import-binding patch 로 검증한다. Phase 7-3 부터 `_summarize_retrieve` 가 query 키워드 매치 위치 주변 forward-bias 윈도우 (top 3 청크 × ±400자, 총 ~1500자 cap) — 짧은 일반 토큰 ("비교", "있는") 이 청크 앞부분에 우연히 걸려 관련 없는 윈도우를 고르는 회귀를 막기 위해 query 토큰을 길이 내림차순 정렬해 긴 토큰부터 매치 시도. 매치가 청크 매우 앞 (`< length//4`) 이면 자연 클램프로 첫 N자 = 7-2 byte-identical, 그 외 구간에서는 윈도우가 매치 위치 따라 이동.
