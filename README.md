...
- `workflows/domains/` — 실제 generic workflow 들과 디스패처 (Phase 6-1). `registry.py` 가 `workflow_key → WorkflowEntry(title / factory / status)` 를 들고 있고, `dispatch.run(key, input)` 이 Phase 5 러너로 실행한다. `general/date_calculation` 이 첫 구현(두 날짜 사이 기간 계산). `workflow_key` 가 비었거나 미등록이면 graph `workflow_node` 가 single_shot 으로 폴백하므로 기존 응답 회귀 0. 어떤 질문을 어떤 key 로 보낼지는 BO `/bo/router-rules/` 의 드롭다운에서 지정 — 코드 키워드 fallback 경로는 key 를 채우지 않는다.
