...
**서비스 레이어**
- `single_shot/` — 질문 하나 → 답변 하나 경로를 단계별 helper 로 분해한 패키지. `pipeline.run_single_shot` 이 외부 진입점이고, 내부에 `retrieval` / `qa_cache` / `prompting` / `llm` / `postprocess` / `types` 가 있음. 공통 규칙(history·error·token·ChatLog·sources·분류)은 `__init__.py` docstring 에 명문화.
...
- `question_router` — 규칙 기반 `route_question(question) -> RouteDecision` (Phase 4-1). 키워드 substring 매칭으로 workflow / agent / single_shot 중 하나를 반환. Phase 4-2 에서 BO RouterRule 이 fallback 앞단으로 들어올 예정.
