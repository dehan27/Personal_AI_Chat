...
- `workflows/domains/` — 실제 generic workflow 들과 디스패처 (Phase 6-1 / 6-2). `registry.WorkflowEntry` 는 `{key, title, description, status, factory, input_schema}` 를 들고 있고, `dispatch.run(key, input)` 이 Phase 5 러너로 실행한다. 현재 구현: `general/date_calculation` (두 날짜 사이 기간) · `general/amount_calculation` (숫자/금액 목록의 합계·평균·차이). `workflow_key` 가 비었거나 미등록이면 `workflow_node` 가 single_shot 으로 폴백 → 회귀 0. BO `/bo/router-rules/` 의 드롭다운에서 `workflow_key` 를 지정하면 해당 질문이 dispatch 로 흐른다. Phase 6-2 부터 `workflow_input_extractor` 가 질문/history 에서 `input_schema` 기반으로 필요한 값을 자동 추출 (regex 우선, 부족하면 gpt-4o-mini fallback).
