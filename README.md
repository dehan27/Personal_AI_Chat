...
- `agent/` — generic agent runtime (Phase 7-1). ReAct loop 가 등록된 도구(`retrieve_documents` / `find_canonical_qa` / `run_workflow`) 를 반복 호출하며 multi-hop 탐색형 질문을 처리하도록 설계. 진입점은 `chat.services.agent.react.run_agent(question, history) -> WorkflowResult`. `Tool.input_schema` 는 `Optional[Mapping[str, FieldSpec]]` 로 schema 모드 / raw 모드 두 갈래 (`run_workflow` 는 raw — workflow_key 마다 input 형태가 달라서). 종료 정책은 `max_iterations=4`, 연속 실패 한도, 동일 호출 반복 한도. 결과는 Phase 5 `WorkflowResult` 그대로 재사용 — 새 dataclass 없음. `AgentTermination` 내부 사유는 `to_workflow_result(...)` 가 `OK / NOT_FOUND / UPSTREAM_ERROR` 중 하나로 매핑한다 (`UNSUPPORTED` 는 라우팅 단의 책임이라 agent 가 만들지 않음). **Phase 7-1 시점 graph 와 미연결** — `ROUTE_AGENT` 는 여전히 single_shot 폴백, Phase 7-2 가 `chat/graph/nodes/agent.py` 로 wrapping.
